...
function FloatingParticles({ count = 150, accent }) {
  const ref = useRef();
  ...
  useFrame((state) => {
    ...
    ref.current.rotation.x = Math.sin(time * 0.006) * 0.04;
    const posArray = ref.current.geometry.attributes.position.array;
    for (let i = 0; i < count; i++) {
      posArray[i * 3 + 1] += 0.0015;
      posArray[i * 3] += Math.sin(time * 0.5 + i) * 0.0002;
      if (posArray[i * 3 + 1] > 5) {
        posArray[i * 3 + 1] = -5;
      }
    }
  ...
  return (
    <div className="contact-canvas">
      ...
      >
        ...
        <FloatingParticles count={150} accent={accent} />
      </Canvas>
    </div>
  );
}
